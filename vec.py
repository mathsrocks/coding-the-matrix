...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    uminusv = {k for k in u.f.keys() if getitem(u, k) != getitem(v, k)}
    vminusu = {k for k in v.f.keys()-u.f.keys() if getitem(v, k) != getitem(u, k)}
    return True if (uminusv | vminusu) == set() else False
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([(getitem(u, k)*getitem(v, k)) for k in u.f.keys()])
